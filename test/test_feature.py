...
def test_insta_post_fetching_data(insta_post):
    ...
    assert_that(len(insta_post.media_urls) > 1, is_(equal_to(True)),
                f"Media from Instagram post {insta_post.__dict__} fetched incorrect.")
    assert_that(insta_post.post_description, is_not(empty()),
